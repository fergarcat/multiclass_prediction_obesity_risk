...
def layout():
    ...
    print(f"DEBUG (prediction.py layout): `fields` recibido: {list(fields.keys() if isinstance(fields, dict) else [])}")
    ...
    if not isinstance(fields, dict) or not fields:
        ...
        print("ERROR (prediction.py layout): `fields` es inválido o vacío.")
    else:
        for field_id, config_dict in fields.items():
            ...
            if processed_field_type == "number":
                ...
                input_component_for_this_field = dcc.Input(
                    id=input_id_current, type='number', value=config_dict.get("default"),
                    min=config_dict.get("min"), max=config_dict.get("max"),
                    step=config_dict.get("step", 0.1), className="form-control"
                )
                ...
                input_component_for_this_field = dcc.Dropdown(
                    id=input_id_current, options=options_list,
                    value=config_dict.get("default"), clearable=False
                )

            if input_component_for_this_field is not None:
                form_elements.append(
                    dbc.Row([
                        dbc.Col(label_component, md=5, className="d-flex align-items-center"),
                        dbc.Col(input_component_for_this_field, md=7)
                    ], className="mb-3")
                )
            else:
                print(f"    WARNING! NO se generó componente para '{field_id}'. Tipo raw: '{raw_field_type}', Tipo procesado: '{processed_field_type}'")

    print(f"DEBUG (prediction.py layout): Número FINAL de `form_elements` generados: {len(form_elements)}")
    ...
    return html.Div(className="page-container-style", children=[
        ...
        dbc.Row(
            dbc.Col(
                dbc.Button('Get Prediction', id='predict-button-prediction', size="lg", className="w-100 mt-4"),
                width=12
            ),
            className="mb-3"
        ),
...
if not isinstance(callback_fields_config, dict) or not callback_fields_config:
    print("CRITICAL ERROR (prediction.py callback setup): `callback_fields_config` es inválido o no se cargó. El callback no funcionará como se espera.")
    callback_fields_config = {}

callback_states = [State(f'input-pred-{field_id}', 'value') for field_id in callback_fields_config.keys()]
print(f"DEBUG (prediction.py callback setup): IDs de State registrados para callback: {[f'input-pred-{field_id}' for field_id in callback_fields_config.keys()]}")
...
def handle_prediction_and_store_results_page(n_clicks, *values):
    ...
    if len(values) != len(field_names_in_order):
        error_msg = "Internal data mismatch. Please contact support."
        is_error = True
        print(f"ERROR (prediction.py callback): Mismatch. Values received: {len(values)}, Expected field names: {len(field_names_in_order)}")
    ...
    print(f"DEBUG (prediction.py callback): Final JSON payload to FastAPI: {input_data}")

    try:
        response = requests.post(client_settings.FASTAPI_PREDICTION_URL, json=input_data)
        response_content = response.json()
        print(f"DEBUG (prediction.py callback): FastAPI Response - Status: {response.status_code}, JSON: {response_content}")
        ...
        is_success = True
        if "Obesity" in pred_label_display or "Overweight" in pred_label_display:
            success_color_class = "warning-custom"
        elif "Normal" in pred_label_display:
            success_color_class = "success-custom"
        else:
            success_color_class = "info-custom"
        ...
        is_error = True
        logging.error(f"Error in prediction.py callback (handle_prediction_and_store_results_page): {e}", exc_info=True)

    if stored_prediction_data is None:
